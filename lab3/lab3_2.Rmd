...
```{r}
class(hot_springs$name)
```
^ takes variable name in data hot_springs
```{r}
hot_springs$scientist <- as.factor(hot_springs$name)
class(hot_springs$name)
```
...
#summary(fish)
```
provides basic info: name of var, class, etc.
