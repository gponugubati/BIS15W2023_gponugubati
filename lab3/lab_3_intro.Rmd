...
```{r}
plants <- c(30.7, 37.6, 28.4, NA, 33.2)
```
...
```{r}
plants2 <- c(4, 5.2, 3.7, NA, 4.6)
```

3. Assemble these vectors into a labeled data matrix with two columns

```{r}
allplants <- c(plants, plants2)
plantmatrix <- matrix(allplants, nrow = 5, byrow = F)
plantmatrix
...
```{r}
heightmean = mean(plantmatrix[,1], na.rm = T)
heightmean
massmean = mean(plantmatrix[,2], na.rm = T)
massmean
```
