...
```r
names(ecosphere) #order, family, common_name, etc. 
```
...
```r
tabyl(ecosphere, order) #19 distinct orders of birds (I counted all the different ones)
```
...
```r
tabyl(ecosphere, habitat) #Woodland has the highest number of species (n = 177)
```
...
```r
ecosphere %>% 
  slice_max(winter_range_area) #Procellariformes
```
...
```r
ecosphere %>% 
  slice_min(winter_range_area) #Passeriformes
```
...
```r
#The Common Elder duck does not live in Wetland. It lives in the Ocean.
```
