...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon, fill = taxon))+ #plots groups within the class (group adds additional categorical variable to plot)
  geom_boxplot(alpha = 0.7)
```
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=5)+ #plots each point with the shape on the line
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.8, color = "black", fill = "lightsteelblue3", bins=40)+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deepskyblue4", alpha  =0.4, color = "black")+ #smooth, not bars
  labs(title = "Distribution of Body Mass")
```
...
```{r message=FALSE}
#IMPORTANT: COMBINES HISTOGRAM WITH SMOOTH LINE
homerange %>% 
  ggplot(aes(x=log10.mass)) +
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(alpha = 0.8, color = "black", fill = "lightsteelblue3", bins=40)+
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "purple3", alpha = 0.4, color = "black")+
...
```{r}
homerange <- homerange %>% #Case When makes continuos data and organizes them as categorical types
