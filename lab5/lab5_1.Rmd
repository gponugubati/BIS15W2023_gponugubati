...
#if it is a character, it changes it to a factor
```
...
```
Pipe is basically passing data to anothe function (cleaner code
)
```{r}
fish %>%
...
```{r}
fish %>% 
  #select(radii_length_mm, scalelength) %>% This step eliminates fish_id column, so next step won't work. ORDER MATTERS
  filter(fish_id == "300") %>% 
...
```{r}
fish %>% 
  mutate(length_mm = length*10) %>% #creates column named length_nm fom length values
...
```{r}
mammals %>%
  mutate_all(tolower) #changes ALL values to lowercase
```
