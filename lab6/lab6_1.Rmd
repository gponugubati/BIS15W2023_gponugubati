...
```{r}
skim(msleep24) #gives statistical info on numerical values in data
```
...
msleep %>% 
  ...
  summarize(mean_sleep_lg = mean(sleep_total)) #makes a new column meansleeplg and calculates mean using existing sleep total column
```
...
msleep %>% 
  filter(order == "Cetacea") #Brain weight of whales is not recorded
```
