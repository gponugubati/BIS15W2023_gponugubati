...
```
Put the lakeid and annnumber in quotes because it is telling R that they are not objects, rather variable names
...
Colon pulls everything between those two  variables
...
Contains() pulls out variables with the word length in it
```{r}
...
```
#There are no NAs. instead, the scientist replaced NAs with the value -999
