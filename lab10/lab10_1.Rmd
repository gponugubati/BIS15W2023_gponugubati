...
life_history %>% 
  ggplot(aes(x = order))+
  geom_bar()+ #To use geom_col, you have to specify y = n in the step above 
  coord_flip()
```
...
life_history %>% 
  ...
  geom_col()+
  coord_flip()+
  scale_y_log10() #scales y values by log 10
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x = reorder(order, mean_mass), y = mean_mass))+ #arranges by value
  geom_col()+
  coord_flip()+
  scale_y_log10() 
```
...
```{r}
elephants %>% 
 # tabyl(sex) #150 females, 138 males
  ggplot(aes(x = sex))+
  geom_bar()
```
...
elephants %>% 
  ...
  labs(title = "Elephants Age vs. Height",
       x = "Age", y = "Height")
```
...
elephants %>% 
  ...
  labs(title = "Elephants Age vs. Height",
       x = "Age", y = "Height")+
  theme(plot.title = element_text(size = rel(1.25), hjust = 0.5)) #change title size and placement
```
...
```{r}
life_history %>% 
  ggplot(aes(x = gestation, y = log10(mass), size = mass))+ #adjusts size of dot by mass
  geom_point()
```
```{r}
life_history %>% 
  filter(gestation >= 20) #big values are elephants
```
