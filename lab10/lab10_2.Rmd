...
```{r}
p+geom_point(aes(shape=thermoregulation, color=thermoregulation), size=1.75) #makes key based on categories listed 
```
...
```{r}
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) + geom_bar(position = "dodge") + #position dodge makes it side by side instead of stacked together
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild")
