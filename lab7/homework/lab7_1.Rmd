...
```{r}
penguins %>% 
  count(sex, island) #can also use tabyl here
```
...
#if you want to include another groups mean, concatenate it as a vector
#summarize(across(c(contains("mm"), body_mass_g), mean, na.rm=T)) This will add body mass mean as a column
```
