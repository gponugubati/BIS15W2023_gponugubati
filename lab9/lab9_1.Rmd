...
```{r}
#indicates addition of layer onto graph
...
ggplot(homerange, mapping = aes(x = log10.mass, y = log10.hra))+
  geom_jitter()+
  geom_smooth(method = lm, se = T) #lm = linear model, se = standard error)
```
...
homerange %>% 
  ...
  geom_col() #At least one variable X or Y has to be categorical in  a bar plot 
```
...
homerange %>% 
  ...
  count(trophic.guild) #more herbivores
```
