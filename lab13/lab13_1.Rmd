...
```{r}
names(spider_list[[14]])
```

## Naming list elements
```{r}
names <- list.files(path = "data/spiders", pattern = ".csv")
names_list = strsplit(names, split = " .csv") #Isolates county names
...
`inner_join`  
Where did customers 3 and 4 go? #Inner only gives output where both columns have common values
```{r}
...
left_exampleDF
```
#Left join matches everything in the left dataset along with whatever matches it on the right data set
