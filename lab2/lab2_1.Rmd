...
**sum function parameters are all objects**
...
```{r}
spdmean = sum(venom_GT, veyron, chiron)/3
```

**remember to use c to concatenate when you have multiple objects**

3. Use the function `mean()` to calculate the mean top speed for the cars.
```{r}
speedmean <- c(venom_GT, veyron, chiron)
mean(speedmean)
```
...
**class() checks for the type of data that the variable is**
...
NAs can thow off data and functions
...
```
**na.rm() removes the NA value in the data**
